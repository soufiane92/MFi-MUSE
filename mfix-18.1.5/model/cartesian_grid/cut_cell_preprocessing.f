MODULE CUT_CELL_PREPROC
   use error_manager, only: flush_err_msg, err_msg
   CONTAINS
...
! Initialize the error manager.
      ...
      IF(RO_G0==ZERO.AND..NOT.DEM_MIO.AND.(.NOT.ENERGY_EQ)) THEN  ! Nothing to do for granular flow and no IC/MIO for DEM
...
!           IF(BC_VELMAG_g(BCV)==UNDEFINED) THEN  ! deprecated key
!
...
!
                  IF (BC_VOLFLOW_G(BCV) /= UNDEFINED) THEN
                     IF (.NOT.COMPARE(VOLFLOW,BC_VOLFLOW_G(BCV))) THEN
                        WRITE(ERR_MSG, 1000) BCV, VOLFLOW, BC_VOLFLOW_G(BCV)
                        call flush_err_msg(abort=.true.)
                     ENDIF
                  ELSE
...
!               IF(BC_VELMAG_s(BCV,M)==UNDEFINED) THEN  !!deprecated key
!
...
!
                     IF (BC_VOLFLOW_S(BCV,M) /= UNDEFINED) THEN
                        IF (.NOT.COMPARE(VOLFLOW,BC_VOLFLOW_S(BCV,M))) THEN
                           WRITE(ERR_MSG,1200)BCV,VOLFLOW,M,BC_VOLFLOW_S(BCV,M)
                           call flush_err_msg(abort=.true.)
                        ENDIF
                     ELSE
