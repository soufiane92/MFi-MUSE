...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
      ...
      use vtk, only: VTK_X_w,VTK_X_e,VTK_Y_s,VTK_Y_n,VTK_Z_b,VTK_Z_t
      use vtk, only: VTK_DEFINED,VTK_FILEBASE
...
! Set teh following to zero to avoid error in WRITE_VTU_FILE loops
      MMAX    = 0
      NMAX(0) = 0
      NSCALAR = 0
      nRR     = 0
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
  SUBROUTINE WRITE_PVD_FRAMES
      ...
      IF(TIME_DEPENDENT_FILENAME) THEN
         ...
         ENDDO
         WRITE(VTU_FRAME_UNIT,*)N_PVD_FILES
         OPEN(UNIT = VTU_FRAME_UNIT, FILE = TRIM(VTU_FRAME_FILENAME))
         N_PVD_FILES=0
         DO L1 = 1, DIMENSION_VTK
            IF(VTK_DEFINED(L1)) THEN
               ...
               WRITE(VTU_FRAME_UNIT,*)N_PVD_FILES,L1,FRAME(L1),TRIM(PVD_FILENAME)
            ENDIF
         ENDDO
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
  SUBROUTINE READ_PVD_FRAMES
      ...
      IF(VTU_FRAME_FILE_EXISTS) THEN
         OPEN(UNIT = VTU_FRAME_UNIT, FILE = TRIM(VTU_FRAME_FILENAME))
         READ(VTU_FRAME_UNIT,*)N_PVD_FILES
         DO L = 1, N_PVD_FILES
            READ(VTU_FRAME_UNIT,*)BUFF1,BUFF2,BUFF3,BUFF_CHAR
            PVD_INDEX(L) = BUFF2
...
! Only open the file from head node when not using distributed I/O
      ...
      IF(TRIM(VTU_DIR)/='.'.AND.TRIM(VTU_DIR)/='') CALL CREATE_DIR(trim(VTU_DIR))
...
! Add the VTU directory path if necessary
      ...
      IF(TRIM(VTU_DIR)/='.'.AND.TRIM(VTU_DIR)/='') & 
            VTU_FILENAME='./'//TRIM(VTU_DIR)//'/'//VTU_FILENAME
...
! Change extension to .vtr (rectilinear mesh)
      ...
      OPEN(UNIT=VTU_UNIT,FILE=VTU_FILENAME,status='unknown')

      WRITE(VTU_UNIT,'(A)') '<?xml version="1.0"?>'
      WRITE(VTU_UNIT,'(A)') '<VTKFile type="RectilinearGrid" &
         &version="0.1" byte_order="LittleEndian">'

      WRITE(VTU_UNIT,'(A,6I6,A)') '<RectilinearGrid &
      ...
      WRITE(VTU_UNIT,'(A,F14.8,A,F14.8,A)') '<DataArray &
         &type="Float32" format="ascii" &
         &RangeMin="',X1,'" RangeMax="',X2,'">'
      WRITE(VTU_UNIT,'(10F14.8)') LIST_OF_X_LINES_DES
      ...
      WRITE(VTU_UNIT,'(10F14.8)') LIST_OF_Y_LINES_DES
      WRITE(VTU_UNIT,'(A)') '</DataArray>'
      ...
      WRITE(VTU_UNIT,'(10F14.8)') LIST_OF_Z_LINES_DES

      WRITE(VTU_UNIT,'(A)') '</DataArray>'
      WRITE(VTU_UNIT,'(A)') '</Coordinates>'
      WRITE(VTU_UNIT,'(A)') '</Piece>'
      WRITE(VTU_UNIT,'(A)') '</RectilinearGrid>'
      WRITE(VTU_UNIT,'(A)') '</VTKFile>'
...
! this is a simple ASCII file
      ...
      IF (myPE == PE_IO.AND.BDIST_IO) THEN
         ...
         IF(TRIM(VTU_DIR)/='.'.AND.TRIM(VTU_DIR)/='') &
               PVTU_FILENAME='./'//TRIM(VTU_DIR)//'/'//PVTU_FILENAME
      ...
      IF(.NOT.PVD_FILE_INITIALIZED(VTK_REGION)) THEN
         ...
         IF(RUN_TYPE == 'NEW'.OR.RUN_TYPE=='RESTART_2')THEN
            ...
            ELSE ! If the pvd file exists, print error message and exits
               WRITE(ERR_MSG,*) TRIM(PVD_FILENAME)//&
                  TRIM(PVD_FILENAME)
               call flush_err_msg(abort=.true.)
            ENDIF
         ELSE
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
  SUBROUTINE UPDATE_AND_CLOSE_PVD_FILE
      ...
      ELSE
         ...
         ENDIF
         IF(TRIM(VTU_DIR)/='.'.AND.TRIM(VTU_DIR)/='') FILENAME='./'//TRIM(VTU_DIR)//'/'//FILENAME
      ENDIF
