         ...
         IF(IS_CG(BC_TYPE_ENUM(ZONE_ID))) THEN
            ...
            DO NN = N1,N2
               ...
               IF(PPFACE<3.OR.PPFACE>4) THEN
                  IF(MyPE == PE_IO) WRITE(ERR_MSG,*)PPFACE, 'POINTS PER FACE. EACH FACE MUST HAVE 3 OR 4 POINTS.'
                  CALL FLUSH_ERR_MSG(ABORT=.TRUE.)
...
!            WRITE(*,*)' ALL FACES READ SUCCESSFULLY.'
      ...
      IF(MyPE == PE_IO) THEN
         WRITE(*,*) ' The file check_geometry.stl was successfully written.'
...
!            IF(STL_BC_ID==BC_PATCH(1)) THEN
               ...
               FLIP_STL_NORMALS(NF1) = FLIP_STL_NORMALS(BC_PATCH(1))
...
! START READING EACH STL FILE, ONE FOR EACH BC_TYPE
      ...
      DO NN = NF1, NF2
         ...
         IF(.NOT.PRESENT) THEN
            IF(MyPE == PE_IO) THEN
               WRITE(*,"('(PE ',I3,'): input data file, ',A20,' is missing: run aborted')") &
...
!      IF(ZMAX_STL>ZLENGTH) ZMAX_STL=ZLENGTH
      ...
      IF(mype.eq.pe_io.and..false.) then
         ...
         IF(MyPE == PE_IO) THEN
            WRITE(*,100) 'The file FACETS_READ.stl was successfully written.'
