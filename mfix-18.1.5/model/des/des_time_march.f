! -*- F90 -*-
! VERSION 2D CONFINEMENT BARBOTEU !
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^!
      ...
      SUBROUTINE DES_TIME_STEP(NN)
        ...
        ! DO P = 1, MAX_PIP
        !    IF ((IGLOBAL_ID(P) == 50) .AND. (PARTICLE_STATE(P) == NORMAL_PARTICLE)) THEN
        !       PRINT'(1X,I4,3(1X,E15.8))',RANK,TC,DES_POS_NEW(P,1),DES_POS_NEW(P,2)
        !    END IF
        !    IF ((IGLOBAL_ID(P) == 70) .AND. (PARTICLE_STATE(P) == NORMAL_PARTICLE)) THEN
        !       PRINT'(1X,I4,3(1X,E15.8))',RANK,TC,DES_POS_NEW(P,1),DES_POS_NEW(P,2)
        !    END IF
        ! END DO
        ! PRINT*,"----------------------------------------------------------------"
...
! ================================================================================= !
        ...
        ! DO P = 1, PIP
        !    IF(IS_NONEXISTENT(P)) CYCLE
        !    PRINT'(3(1X,I4),4(1X,E15.8))',IGLOBAL_ID(P),PARTICLE_STATE(P),RANK,DES_POS_NEW(P,1),DES_POS_NEW(P,2)!,DES_USR_VAR(AS_RO_SOL,P),DES_USR_VAR(AS_PMASS,P)
        ! END DO
...
! ------------------------------------------------------------------------------------------------------------- !
           ...
           DO P = 1, MAX_PIP
              ...
              DO CC = CC_START, CC_END-1
                 ...
                 IF ( (PARTICLE_STATE(P) * PARTICLE_STATE(I) == 4) .OR. (PARTICLE_STATE(I) * PARTICLE_STATE(P) == 4) .OR. &
                      (PARTICLE_STATE(P) * PARTICLE_STATE(I) == 5) .OR. (PARTICLE_STATE(I) * PARTICLE_STATE(P) == 5) .OR. &
                      (PARTICLE_STATE(P) * PARTICLE_STATE(I) == 6) .OR. (PARTICLE_STATE(I) * PARTICLE_STATE(P) == 6) ) THEN 
                    ...
                    IF (IGLOBAL_ID(P) .NE. IGLOBAL_ID(I)) THEN
                       ...
                       IF (SS .LT. 1D-8) THEN ! DISTANCE PREDICTION LEAPFROG
                          ...
                          ! MEQ = 1.63362817986669
                          ! DES_USR_VAR(AS_PMASS,P) = 2.D00*MEQ
                          ! DES_USR_VAR(AS_PMASS,I) = 2.D00*MEQ
                       ...
                       ! IF (PARTICLE_STATE(P) == 4 .OR. PARTICLE_STATE(P) == 5 .OR. PARTICLE_STATE(P) == 6) THEN
                          !> SANS FROTTEMENT
                       ...
                       ! IF (PARTICLE_STATE(P) == 4 .OR. PARTICLE_STATE(P) == 5 .OR. PARTICLE_STATE(P) == 6) THEN
                          ...
                          !> BARBOTEU: EULER
                          DES_USR_VAR(AS_NEW_X,I) = DES_USR_VAR(AS_OLD_X,I) + DTSOLID*DES_USR_VAR(AS_NEW_U,I)
                          DES_USR_VAR(AS_NEW_Y,I) = DES_USR_VAR(AS_OLD_Y,I) + DTSOLID*DES_USR_VAR(AS_NEW_V,I)
                          DES_USR_VAR(AS_NEW_Z,I) = DES_USR_VAR(AS_OLD_Z,I) + DTSOLID*DES_USR_VAR(AS_NEW_W,I)
                       END IF
                       ! ================================================================================================================ !
...
! ------------------------------------------------------------------------------------------------------------- !
           ...
           DO P = 1, MAX_PIP
              ...
              DO CC = CC_START, CC_END-1
                 ...
                 IF ((IGLOBAL_ID(P) == 50) .AND. (PARTICLE_STATE(P) == NORMAL_PARTICLE)) THEN
                    IF (TC .GT. 0.196 .AND. TC .LT. 0.331) THEN
                       PRINT'((1X,E15.8),2(1X,I4),4(1X,E15.8))',TC,RANK,IGLOBAL_ID(I),DES_POS_NEW(P,1),DES_POS_NEW(P,2),DES_POS_NEW(I,1),DES_POS_NEW(I,2)
                    END IF
                 END IF

                 IF ( (PARTICLE_STATE(P) * PARTICLE_STATE(I) == 1) .OR. &
                      (PARTICLE_STATE(P) * PARTICLE_STATE(I) == 2) .OR. &
                      (PARTICLE_STATE(P) * PARTICLE_STATE(I) == 3) ) THEN
