...
mpirun -np 1 "${MFIXSOLVER}" -f mfix.dat imax=8 jmax=8 kmax=8 DISCRETIZE=9*2 UR_FAC=9*0.05 >out.log
...
mpirun -np 1 "${MFIXSOLVER}" -f mfix.dat imax=16 jmax=16 kmax=16 >out.log
cat "$CASE_DIR"/de_norms.dat >>"$CASE_DIR"/de_norms_collected.dat
rm -f "$CASE_DIR"/{MMS05.*,de_norms.dat,out.log}
rm -f "$CASE_DIR"/solution_*.dat
