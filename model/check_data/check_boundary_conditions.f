MODULE CHECK_BOUNDARY_CONDITIONS_MOD
      ...
      use bc, only: bc_type_enum, mass_inflow, p_inflow, outflow, mass_outflow, p_outflow, free_slip_wall, no_slip_wall, par_slip_wall, bc_defined, dummy, is_cg
...
! Only check geometry when doing PPO
      IF(PPO) THEN
...
! Determine which solids phases are present.
            ...
            CASE (MASS_INFLOW)
               ...
               CALL CHECK_BC_MASS_FLOW(MMAX_TOT, SKIP, BCV)
            ...
            CASE (MASS_OUTFLOW)
               ...
               CALL CHECK_BC_MASS_FLOW(MMAX_TOT, BCV)
               CALL CHECK_BC_OUTFLOW(MMAX_TOT, BCV)
