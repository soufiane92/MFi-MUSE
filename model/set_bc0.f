      ...
      DO L = 1, DIMENSION_BC
         IF (BC_DEFINED(L)) THEN
            ...
            CASE (P_OUTFLOW)
               ...
               CALL SET_BC0_EP(L)
               CALL SET_BC0_SCALARS(L)
            CASE (MASS_OUTFLOW)
               CALL SET_BC0_DT_CALCS(L)
               CALL SET_BC0_OUTFLOW(L)

               CALL SET_BC0_EP(L)
               CALL SET_BC0_MASS_OUTFLOW(L)
               CALL SET_BC0_SCALARS(L)
            CASE (OUTFLOW)
               CALL SET_BC0_DT_CALCS(L)
               CALL SET_BC0_OUTFLOW(L)
...
!  they are defined) for pressure outflow (PO), outflow (O), and       C
!  mass outflow (MO) boundary types. An additional routine is needed   C
!  to address additional quantities needed by the MO boundary.         C
!                                                                      C
...
!                                                                      C
!  This can cause some issue if we have variable density and need to   C
!  calculate ro_g in the boundary which could be used for a PO         C
!  boundary.                                                           C
!                                                                      C
...
!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvC
!                                                                      C
!  Subroutine: set_bc0_mass_outflow                                    C
...
!                                                                      C
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
      SUBROUTINE SET_BC0_MASS_OUTFLOW(BCV)
...
! needs to be set for both sides of the boundary cell.
      ...
      RETURN
      END SUBROUTINE SET_BC0_MASS_OUTFLOW
...
!  Purposes: In certain circumstances, mass in/outflow boundaries may  C
!  require additional handling. Namely, bc_ep_g and bc_rop_s are not   C
!  strictly needed when the corresponding phase velocity is defined    C
!  at the boundary. However, the check_data_20 routine does not        C
!  discriminate requiring ep_g and rop_s are defined for all non-wall  C
!  cells. So, this routine sets ep_g and/or rop_s in MI/MO boundary    C
!  cells according to the neighbor fluid cell when bc_ep_g and/or      C
!  bc_rop_s are not defined..                                          C
!                                                                      C
!  Note, that these values should not be needed by the solver during   C
!  the simulation for MI/MO type boundaries. The velocities are set    C
!  in these cells per the initial setup; only the conversion of a      C
!  mass flow and/or volume flow to velocity necessitates such values.  C
!                                                                      C
!  Similarly, PO, O, and PI boundaries may or may not have these       C
!  quantities set. Undefined values are effectively overcome by a      C
!  later call in set_bc1, however, by doing so here we permit the      C
!  earlier calls to set_ro_g and set_ro_s in the instance of a new     C
!  run. Those routines loop over boundary cells and so require ep_g &  C
!  maybe rop_s. Otherwise one must wait for set_bc1 to called wherein  C
!  these boundary values become set to their neighbor fluid values..   C
!                                                                      C
...
!--------------------------------------------------------------------//
      ...
      use functions, only: is_on_mype_plus2layers, funijk
      use compar, only: dead_cell_at
      IMPLICIT NONE
...
!                                                                      C
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^!
      SUBROUTINE set_bc0_scalars(BCV)
...
! this should only loop of tfm phases .. hmm
      ...
      ENDDO
      RETURN
      END SUBROUTINE set_bc0_scalars
