...
! status of the vtp file to be written
      ...
      IF(TRIM(VTP_DIR)/='.'.AND.TRIM(VTP_DIR)/='') CALL CREATE_DIR(trim(VTP_DIR))
...
! Second pass writes the data (appended binary format).
      ...
      DO PASS=WRITE_HEADER,WRITE_DATA
         ...
         IF(VTK_PART_DIAMETER(VTK_REGION).AND.ALLOCATED(DES_RADIUS)) &
...
! Update Frames
     ...
     IF (FULL_LOG.AND.myPE == PE_IO) THEN
        IF(GLOBAL_CNT>0.AND.MODE==0) THEN 
           WRITE(*,20)' DONE.'
        ELSE
           WRITE(*,20)' VTP file not written (zero particle in vtk region).'
        ENDIF
     ENDIF
