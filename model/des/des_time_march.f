...
! ------------------------------------------------------------------------------------------------------------- !
           ...
           !             TAU_N = PN(CC) - GAMMA * SS
                       ...
                       ! IF (TAU_N .GE. -1.D-10) THEN
           !                DPN(CC) = - VM_N(CC) / W(1,1) !* (1.D0 + EN)
           !                ACTIVE(CC) = .TRUE.
           ...
           !                DES_USR_VAR(AS_NEW_W,P) = DES_USR_VAR(AS_NEW_W,P) - DPN(CC)*DTSOLID/DES_USR_VAR(AS_PMASS,P) * NZ_AS
           ...
           !                DES_USR_VAR(AS_NEW_W,I) = DES_USR_VAR(AS_NEW_W,I) + DPN(CC)*DTSOLID/DES_USR_VAR(AS_PMASS,I) * NZ_AS
...
! ------------------------------------------------------------------------------------------------------------- !
              ...
              DO CC = CC_START, CC_END-1
                 ...
                 IF ((PARTICLE_STATE(P) * PARTICLE_STATE(I) == 1)) THEN
                    ...
                    IF (IGLOBAL_ID(P) .NE. IGLOBAL_ID(I)) THEN
                       ...
                       ! OPERATEURS D'INERTIE DE CYLINDRES (AXE Z) : mettre billes en 3D !!
                       P_OPINERTI=PMASS(P)*DES_RADIUS(P)*DES_RADIUS(P)/2.D00
                       I_OPINERTI=PMASS(I)*DES_RADIUS(I)*DES_RADIUS(I)/2.D00
                       
                       IMEFF(1) = DTSOLID/PMASS(I)
                       IMEFF(2) = DTSOLID/PMASS(I)
                       IMEFF(3) = DTSOLID/I_OPINERTI
                       
                       PMEFF(1) = DTSOLID/PMASS(P)
                       PMEFF(2) = DTSOLID/PMASS(P)
                       PMEFF(3) = DTSOLID/P_OPINERTI
           ...
           END DO
           ! PRINT*,"----> ",NLGS,DES_USR_VAR(AS_NEW_X,1),DES_USR_VAR(AS_NEW_U,1)
           ! PRINT*,"++++> ",NLGS,DES_USR_VAR(AS_NEW_T,1),DES_USR_VAR(AS_NEW_DT,1)
           DO_NSEARCH = (NN == 1 .OR. MOD(NN,NEIGHBOR_SEARCH_N) == 0)
           IF (DO_NSEARCH .OR. (NUMPES>1) .OR. DES_PERIODIC_WALLS) THEN
              CALL DESGRID_PIC(.TRUE.)
              CALL DES_PAR_EXCHANGE
