...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^!
      SUBROUTINE DES_TIME_INIT
        ...
        ! PRINT*,EN,DTSOLID,GRAV(2),RADIUS
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^!
      ...
      SUBROUTINE DES_TIME_STEP(NN)
        ...
        DO P = 1, MAX_PIP
           ...
           !> EN 2D
           PMASS(P) = RO_SOL(P) * ACOS(-1.0) * DES_RADIUS(P)**2
...
! ======================================================================================= !                
        ...
        DO P = 1, PIP
           IF ( PARTICLE_STATE(P) == 1 ) THEN
              ...
              ! CALL GET_SIGNED_DISTANCE([0.0D0,0.4D0],[2.0D0,0.4D0],DES_POS_NEW(P,1:2),N_CL_INF,DIST_CL_INF)
              ...
              ! CALL GET_SIGNED_DISTANCE([0.4D0,0.4D0],[0.4D0,2.0D0],DES_POS_NEW(P,1:2),N_CL_LFT,DIST_CL_LFT)
              ! CALL GET_SIGNED_DISTANCE([0.2D0,0.4D0],[0.2D0,0.8D0],DES_POS_NEW(P,1:2),N_CL_LFT,DIST_CL_LFT)
              ...
              ! CALL GET_SIGNED_DISTANCE([1.8D0,0.4D0],[1.8D0,2.0D0],DES_POS_NEW(P,1:2),N_CL_RGHT,DIST_CL_RGHT)
...
! ------------------------------------------------------------------------------------------------------------- !
        ...
        !       PRINT*,NLGS,IGLOBAL_ID(P),IGLOBAL_ID(I),PN_ARRAY(CC),ARRAY_SS(CC)!,DES_USR_VAR(AS_NEW_Y,6),DES_USR_VAR(AS_NEW_Y,1)
        !    END DO
        ! END DO
...
! ========================================================================== !                
        ...
        !> FRONTAL COLLISION VALIDATION 
        ! DO P = 1, 2
        !    WRITE(41,*)DES_VEL_NEW(P,1),VEL_ANALYTIC(P,1),ABS(VEL_ANALYTIC(P,1)-DES_VEL_NEW(P,1)),NSTEP*DTSOLID
        !    WRITE(42,*)DES_VEL_NEW(P,2),VEL_ANALYTIC(P,2),ABS(VEL_ANALYTIC(P,2)-DES_VEL_NEW(P,2)),NSTEP*DTSOLID
        ! END DO
